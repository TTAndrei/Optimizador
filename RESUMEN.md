...
## Estado actual (2026-08-10, rama Malla-Variable, commit `a7dc70b`)

**SEGUNDA GRAN OPTIMIZACIÓN A MITAD DE CAMINO Y PAUSADA** (épocas 0–7; ver bloque al final de esta sección). Todo lo demás de abajo sigue vigente.
...
**Reorganización de resultados**: toda la campaña anterior está archivada en **`results/1eraGranOptimizacion/`** (`convergence_study/`, `verificacion_numerica/`, `agent_tests/`, `barridos/`, `metricas_ga/`, `comparativas_v1/`, `videos_ganadores/`, …). `results/verificacion_numerica/` en la raíz solo contiene lo nuevo (`criterio_parada.json/.png`, `series_dx004/`, `espesor_vs_malla.json`). Cualquier ruta de este documento o de scripts antiguos que apunte a `results/convergence_study/...` hay que leerla bajo `results/1eraGranOptimizacion/`. El `.gitignore` se reancló a patrones `**/` porque los anclados a `results/` dejaron de aplicar tras el archivado (metían 5585 ficheros y tres >100 MB al índice).
...
### Segunda gran optimización (lanzada 2026-08-04, PAUSADA en la época 7)

Campaña de días con el criterio de parada validado. Lanzador `scripts/agent_tests/gran_optimizacion.sh {start|run|stop|status|snapshot|log}`; salidas en **`results/convergence_study/islands_tfg2/`**, log `results/convergence_study/gran_optimizacion_tfg2.log`, PID en `..._tfg2.pid`, copias locales en `results/convergence_study/backups/tfg2_*`.
...
- **Pausa/reanudación**: `stop` crea el centinela `STOP`; la corrida termina la evaluación CFD en curso (≤12 min), guarda `estado_ga.json` y sale. `start` continúa exactamente donde quedó — las épocas cerradas se saltan y los individuos ya simulados de la generación a medias no se vuelven a pagar. `guardar_estado_ga` escribe a `.tmp` + fsync + `os.replace` (`2d2d639`), así que un corte de luz deja el checkpoint viejo o el nuevo, nunca uno truncado; antes un JSON a medias hacía caer la reanudación en "checkpoint ilegible; empiezo de cero" y perdía la época entera.

**Progreso (a `a7dc70b`, 2026-08-10): 31 de 52 corridas GA cerradas, ~120 h de las 311 presupuestadas.** Épocas 0–7 en `GM15`, `AG24` y `NACA_0012_sharp`; 0–6 en `s1014` (la 7 quedó pausada antes de su primera generación, no se pierde nada más que el arranque). Reanudar con `gran_optimizacion.sh start`.

Mejor L/D por isla y época **a dx=0.004** (sesgo absoluto −44.9 %, sirve para ordenar, no como valor final):

| época | GM15 | AG24 | NACA_0012_sharp | s1014 |
|---|---|---|---|---|
| 0 | 16.96 | 15.61 | 14.71 | 7.14 |
| 1 | 19.90 | 19.28 | 17.70 | 8.03 |
| 2 | 21.81 | 21.49 | 19.82 | 21.77 |
| 3 | 22.90 | 22.09 | 22.65 | 22.71 |
| 4 | 23.13 | 22.70 | 22.66 | 23.29 |
| 5 | 24.85 | 23.06 | 23.22 | 25.27 |
| 6 | 25.44 | 24.73 | 24.95 | **25.66** |
| 7 | **25.75** | **25.58** | **24.98** | (pendiente) |

- **La "meseta hacia L/D ~23.3" que se leyó tras la época 4 era falsa**: la ganancia por época venía dividiéndose por dos (+2.94, +1.92, +1.09, +0.23) y las cuatro islas estaban dentro de 0.48 puntos, pero las épocas 5–7 dieron otros **+2.6**. No dar por convergida una campaña de islas por dos épocas planas seguidas — la migración reordena la población con retardo.
- **s1014 arrancaba en otra cuenca** (distancia de forma ~0.09 frente a ~0.006 entre las otras tres) y tardó tres épocas en integrarse; al hacerlo el spread de L/D cayó de 11.87 a 2.00 y la distancia de forma media de 0.0562 a 0.0057. La migración mueve una isla lejana, pero con varias épocas de retardo. Confirma el papel de la migración ya diagnosticado en el estudio de convergencia.
- Dataset ML acumulado: **4698 registros** en `aprendizaje_ML.jsonl` (+814 desde el arranque de la campaña).

**El adelgazamiento del perfil NO es artefacto de malla** (`scripts/agent_tests/test_espesor_malla.py` → `results/verificacion_numerica/espesor_vs_malla.json`). El GA lleva el espesor máximo del 12 % (semilla) a **~1.9 %** con el L/D subiendo monótonamente, y a dx=0.004 un cuerpo del 2 % solo tiene ~5 celdas de espesor: la sospecha era que el optimizador estuviera explotando la falta de resolución. Cuatro ganadores del mismo linaje reevaluados a dx=0.002:
...
**El orden se conserva** entre mallas y la mejora del linaje solo se encoge de **1.513× a 1.371×** (mejor sobre e0_GM15). El adelgazamiento es física de Re bajo — el Cd cae a la mitad mientras el Cl solo baja de 1.05 a 0.73 —, no error de discretización.
...
**Estado del árbol**: todo lo importante commiteado en `a7dc70b` (checkpoints, ganadores, historiales y log de las épocas 0–7, `aprendizaje_ML.jsonl`, `polar_results.json`). Sin seguir: `RESUMEN.md.tmp` (artefacto del hook de regeneración). **`cerebro_aerodinamico.pkl` sale modificado y se deja deliberadamente fuera de los commits de campaña**: el fichero en disco es el truncado por la prueba de humo (74 experiencias frente a las 2347 de `8ab2461`) y se restaurará desde el histórico de git al cerrar la campaña — no commitear esa versión. Los pendientes de limpieza de julio (rename `core_v0x`→`v0x_ic`, `--smoke`/Cl_cp en scripts de barrido, `prueba_inicialesdiferentes/`, `comparativas_v1/`, reorganización de `convergence_study/`) están resueltos vía el archivado en `results/1eraGranOptimizacion/`.
...
**Prioridad (2026-08-10)**

1. **Terminar `islands_tfg2`**: quedan 21 de 52 corridas (~190 h). `./scripts/agent_tests/gran_optimizacion.sh start` reanuda por s1014/e7; `status` da épocas cerradas y mejor L/D por isla; `stop` pausa limpio; `snapshot` copia sin parar. Al terminar: refinado automático top-3 a dx=0.002 (`refine_dx002.json`), `metricas_ga.py` sobre la corrida nueva, y **restaurar `cerebro_aerodinamico.pkl` desde `8ab2461`** (el de disco está truncado a 74 experiencias). **No dar la campaña por convergida por dos épocas planas** — ya pasó tras la época 4 y luego llegaron +2.6 puntos.
2. ~~Relanzar la optimización con el criterio nuevo~~ → **HECHO, a mitad** (épocas 0–7).
3. ~~Manejar fitness negativo antes de lanzar~~ → **HECHO** (`5213410`): centinela `Individuo.evaluado`, filtro `ld > 0` eliminado, pool por `ind.resultados`.
4. ~~Comprobar si el adelgazamiento a ~1.9 % de espesor es artefacto de dx=0.004~~ → **HECHO** (`a7dc70b`, `test_espesor_malla.py`): no lo es, el orden entre mallas se conserva.
5. **Diagnosticar el exceso de Cd** (+226 % a α=10 frente a XFOIL): perfil de capa límite en pared a dx=0.001 vs dx=0.002, y posición de reataque de la burbuja frente a lo que predice XFOIL. Decide entre las hipótesis 1 y 2 de §5.2 (resolución de BL vs separación prematura). **Gana peso** ahora que el GA converge a cuerpos muy finos: si el Cd está sesgado, el óptimo de espesor lo está también.
6. **GA multipunto α∈{2,4,6}**: se descartó cuando una evaluación costaba ~17 min a dx=0.002 con presupuesto fijo; con dx=0.004 y parada por meseta baja a ~12 min por los tres ángulos, así que vuelve a estar sobre la mesa. Etapa `multipunto` ya cableada en `cola_tfg.sh` (fuera de la cola por defecto, hay que lanzarla a mano).
7. **Activar el surrogate** (`usar_ia=True`, umbral percentil 70): ahorro estimado del 69 % del CFD con pérdida del 2.5 % de la élite. Reentrenar sobre datos medidos con el criterio nuevo — los 814 registros nuevos de `tfg2` son esa fuente limpia; no reentrenar sobre el historial sesgado.
8. **Terminar la memoria del TFG** (`docs/tfg/memoria.txt` → `build_tfg.py`), incorporando los resultados de `tfg2` cuando cierre.

**Anteriores (siguen abiertos)**

9. Estudio de convergencia CERRADO: no relanzar corridas de diagnóstico de la misma pregunta. Direcciones abiertas: más diversidad/multi-arranque si se quiere un óptimo global real; revisar físicamente el óptimo de camber alto (Cl~1.07 a α=4, semilla s1014).
10. **Ejecutar el barrido** `scripts/agent_tests/run_cfl_sweep.py` (30 sims, resumible) para caracterizar coste/error CFL×dx×α — sigue pendiente, ninguna sim lanzada.
11. Investigar el error no fatal de carga de `cerebro_aerodinamico.pkl` ("inhomogeneous shape") — **sigue apareciendo en cada arranque de isla de `tfg2`** ("Memoria recuperada: 74 experiencias" → error → reinicia). No bloquea y con `guardar_memoria` arreglado (`5213410`) ya no borra el `.pkl`, pero mientras no se diagnostique el surrogate arranca en frío en cada época.
12. Opcional barato: probar MacCormack+turbo_hd (~19 it/s esperado) para pre-screening si el sesgo por Q=0.026 resulta consistente entre geometrías.
13. Opcional física: sublinealidad α=8 (deriva -0.002, TE separación creciendo) — vigilar si el optimizador explora α altos.
14. Actualizar rutas en scripts/notebooks que apunten a `results/convergence_study/...` o `results/verificacion_numerica/{gci,polar,reeval_*}.json` — los de la campaña vieja viven bajo `results/1eraGranOptimizacion/`. **Cuidado**: `results/convergence_study/` volvió a existir y ahora contiene la campaña NUEVA (`islands_tfg2/`), así que una ruta antigua ya no falla — apunta a datos distintos.

## Tests
- `scripts/agent_tests/gran_optimizacion.sh {start|run|stop|status|snapshot|log}` — segunda gran optimización (islas `tfg2`, dx=0.004, 13 épocas, tope 311 h). `start` en segundo plano con PID, `run` en primer plano (Ctrl+C pausa), `stop` deja el centinela `STOP` y espera al checkpoint (≤12 min), `snapshot` copia el estado a `backups/` sin parar la corrida. Reanuda sin repetir evaluaciones pagadas.
- `scripts/agent_tests/test_espesor_malla.py` — reevalúa a dx=0.002 ganadores de distintas épocas de una isla para separar física de artefacto de malla → `results/verificacion_numerica/espesor_vs_malla.json`.
...
- `scripts/agent_tests/video_ganador.py` — vídeo del campo del ganador. `plot_comparacion_transicion.py` — comparativa SA vs SA-BC. `validar_aislado_dx002.py` — revalidación a dx=0.002 de los ganadores del test aislado. `t1_ranking_dx.py` — test T1 de ranking entre mallas.
