"""
Polar fina (alpha = 0..10 en pasos de 1 grado) de los dos perfiles del TFG a
dx=0.002, Re=1e5: el ganador de islands_tfg2/epoch10 y el NACA 0012 de
referencia.

Misma ruta de cálculo que la polar de 2 grados (verificacion_numerica.simular):
config de referencia consistent + SA + SA-BC + MacCormack y criterio de parada
calibrado. Una simulación independiente por (perfil, ángulo).
...
Uso:
    .venv/bin/python scripts/agent_tests/polar_fina.py
    .venv/bin/python scripts/agent_tests/polar_fina.py --solo-figuras
"""
...
OUT = os.path.join(ROOT, "results", "polar_fina_1grado")
CAMPOS = os.path.join(OUT, "campos")
FIGS = os.path.join(OUT, "figuras")
for d in (OUT, CAMPOS, FIGS):
    os.makedirs(d, exist_ok=True)

STOP = os.path.join(ROOT, "STOP_SIMULATION.trigger")

ANGULOS = tuple(float(a) for a in range(0, 11))
RE = 1e5
DX = 0.002
...
COLS = ["alpha", "cl", "cd", "ld", "cl_ci95", "cd_ci95", "ld_ci95",
        ...
        "cl_cp_discrepancy_flag", "dx", "Re", "wall_s"]

# Recorte alrededor del perfil: la malla fina cubre 1.5x1.0 centrada en el
# cuerpo, que va de x=cx a x=cx+chord con cy=Ly/2.
CX, CY = 2.0, 2.5
ZOOM_X = (CX - 0.45, CX + 1.85)
ZOOM_Y = (CY - 0.70, CY + 0.70)

# Escala de color común a todos los ángulos y a los dos perfiles: sin ella cada
# imagen se autoescala y dejan de ser comparables entre sí.
VMAX = 1.8
...
def figuras_campo(tag, a, meta):
    ...
    cab = (f"{lab} — α={a:.0f}°, Re=1e5, dx=0.002\n"
...
def figuras_polar(datos):
    ...
    for col, err, ylabel, titulo, salida, paso in GRAFICOS:
        ...
        ax.set_title(f"{titulo} — Re = 100 000, dx = 0.002")
...
def main():
    ...
    for a in ANGULOS:
        for tag, meta in PERFILES.items():
            ...
            r = vn.simular(meta["dat"], RE, DX, alpha=a,
                           dump_field=campo_path(tag, a))
            if r is None:
